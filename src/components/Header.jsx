...
const Header = () => {
  ...
  return /*#__PURE__*/<header className={`sticky top-0 z-50 w-full transition-all duration-300 ${scrolled ? "bg-white/85 backdrop-blur-lg border-b border-sky-100 shadow-sm" : "bg-transparent"}`}>{/*#__PURE__*/<div className="max-w-7xl mx-auto px-5 md:px-8 h-20 md:h-[110px] flex items-center justify-between">{/*#__PURE__*/<Link to="/" className="flex-shrink-0 flex items-center group mr-8 md:mr-16">{/*#__PURE__*/<img src="/logo.png" alt="Kriscel Tech" className="h-[112px] md:h-[164px] my-[-16px] md:my-[-27px] w-auto max-w-none block object-contain group-hover:scale-105 transition-transform" />}</Link>}{/*#__PURE__*/<nav className="hidden lg:flex items-center gap-1">{nav.map(n => /*#__PURE__*/<NavLink to={n.to} className={({
          ...
          isActive
        }) => `block px-4 py-3 rounded-lg text-sm font-medium ${isActive ? "text-sky-600 bg-sky-50" : "text-slate-700 hover:bg-sky-50/60"}`}>{n.label}</NavLink>)}{/*#__PURE__*/<div className="pt-3 mt-3 border-t border-slate-100">{/*#__PURE__*/<p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest px-4 mb-2">Management Systems</p>}{mock.managementSystems.map((s, i) => /*#__PURE__*/<Link to="/products" className="block px-4 py-2 text-sm text-slate-600 hover:text-sky-600 hover:bg-sky-50/60 rounded-lg">{s.title}</Link>)}</div>}{/*#__PURE__*/<Link to="/contact">{/*#__PURE__*/<Button className="w-full mt-3 bg-slate-900 hover:bg-slate-800 text-white rounded-full h-11 font-semibold">Book a Demo</Button>}</Link>}</div>}</div>}</header>;
};
